...
//	std::cout << net_->num_outputs() << std::endl;
  ...
  feat1 = new float[cnn_model_.lastfm_len()];
  feat2 = new float[cnn_model_.lastfm_len()];
...
std::vector<float> Classifier::FPGA_Predict(const cv::Mat& sample_resized){
  ...
  printf("FPGA Prediction Start\n");
    ...
    img1 = new float[cnn_model_.infm_len()];
    for(int i=0; i<cnn_model_.infm_len(); i++) {
  ...
  */
  float *ddram = (float*)malloc(sizeof(float)*cnn_model_.infm_len());
  const float* fdata = net_->input_blobs()[0]->cpu_data();
  for(int i=0; i<cnn_model_.infm_len(); i++) {
    ddram[i] = (float)fdata[i];
  }

  vector< float > result = fpga_.FPGAexec(ddram);
  float* fpga_feat = (float*)malloc(sizeof(float)*result.size()); 
  for(int i=0; i<result.size(); i++){
    fpga_feat[i] = result[i]; 
  }
  cnn_model_.ann(net_, fpga_feat, fpga_result);
